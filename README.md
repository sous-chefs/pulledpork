...
You will need to set the rule_url_array attribute for pulledpork to run. The attribute defaults to an empty array to give full control over the rules you run. Kitchen CI leaves the array empty so convergence does not depend on rate-limited external rule downloads.
